...
function previewEditor (event) {
    ...
    console.log('mdPreview', event.target.prev);
}
...
function editingPage (elem, destinationElement) {
    ...
    var titleDiv, markdownPreview, markdownPreviewTitle;
    if(td) {
        titleDiv = td;
...
function editingReset(elem, destElem) {
...
function initialize(run) {
    ...
    if (location.href.indexOf('/apex/adm_bugedit') > -1 && location.href.indexOf('gus.lightning.force') > -1) {
        ...
        if(run) {
            ...
            editingReset(element, destinationElement);
        }
